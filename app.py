...
# ✅ Function to get AI-based description of the image
def generate_description(image):
    try:
        response = model.generate_content(["Describe this image in detail for a blind person in 40 words:", image])
        return response.text if response else "No description available"
...
# ✅ Define a video processing class
class VideoProcessor(VideoTransformerBase):
    def __init__(self):
        self.frame_count = 0  # Process every Nth frame to avoid performance issues
    
    def transform(self, frame: av.VideoFrame) -> np.ndarray:
        self.frame_count += 1
        image = frame.to_ndarray(format="bgr24")

        # Process every 100 frames (~every 3-5 sec)
        if self.frame_count % 100 == 0:
            try:
                ...
                description = generate_description(img_pil)
                st.session_state["latest_description"] = description  # Store latest description
            except Exception as e:
                st.session_state["latest_description"] = f"Error: {e}"

        return image  # Return original frame

# ✅ Streamlit UI
st.title("📹 Vocal Eyes - Live Video")

# Display the AI-generated description in real-time
if "latest_description" not in st.session_state:
    st.session_state["latest_description"] = "Waiting for AI response..."

st.write(f"**🔍 AI Description:** {st.session_state['latest_description']}")

# ✅ Start the webcam stream
webrtc_streamer(key="video_feed", video_processor_factory=VideoProcessor)
